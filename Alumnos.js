...
[
  ...
  return (
    <View style={styles.contenedor}>
      <FlatList
        data={estudiantes}
        keyExtractor={(elemento) => elemento.matricula.toString()}
        renderItem={({ item: elemento }) => (
          <View style={styles.tarjeta}>
            <Text style={styles.nombreEstudiante}>{elemento.nombre}</Text>
            <Text style={styles.matriculaEstudiante}>Matrícula: {elemento.matricula}</Text>
          </View>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  tarjeta: { padding: 20, borderBottomWidth: 1, borderBottomColor: '#eee' },
